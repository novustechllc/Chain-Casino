...
module casino::IntegrationTest {
    ...
    // Helper function to get stored game capability for testing
    fun get_game_capability(): &GameCapability {
        borrow_global<GameCapability>(@casino)
    }
    ...
    fun setup_integration_test(): (signer, signer, signer, signer) {
        ...
        // Register test game and store capability
        let capability =
        ...
        // Store capability for testing
        move_to(&casino_account, capability);
    ...
    fun test_nav_increase_from_game_profit() {
        ...
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(
            capability, bet_coins, @0x111, profit_amount * 2
        );

        // Settle with house profit (no winner payout)
        CasinoHouse::settle_bet(capability, bet_id, @0x0, 0);
    ...
    fun test_multiple_game_rounds_nav_accumulation() {
        ...
        while (i < 3) {
            ...
            let capability = get_game_capability();
            let bet_id = CasinoHouse::place_bet(
                capability, bet_coins, @0x111, GAME_BET * 2
            );
            CasinoHouse::settle_bet(capability, bet_id, @0x0, 0);
    ...
    fun test_game_payout_exceeds_treasury() {
        ...
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(
            capability, bet_coins, @0x111, GAME_BET * 2 + 1
        );

        // Try to payout more than treasury has
        CasinoHouse::settle_bet(capability, bet_id, @0x111, GAME_BET * 2 + 1);
    }
    ...
    fun test_treasury_boundary_conditions() {
        ...
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(capability, bet_coins, @0x111, payout_amount);
        CasinoHouse::settle_bet(capability, bet_id, @0x111, payout_amount);
    ...
    fun test_concurrent_redemption_and_game_settlement() {
        ...
        let bet_coins = coin::withdraw<AptosCoin>(&casino_account, GAME_BET);
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(capability, bet_coins, @0x111, GAME_BET * 2);
    ...
    fun test_multiple_investor_operations_with_games() {
        ...
        let capability1 = get_game_capability();
        let bet_id1 = CasinoHouse::place_bet(
            capability1, bet_coins1, @0x111, GAME_BET * 2
        );

        InvestorToken::deposit_and_mint(&investor2, TEST_DEPOSIT * 2);

        // Settle game 1
        CasinoHouse::settle_bet(capability1, bet_id1, @0x0, 0);
        ...
        let capability2 = get_game_capability();
        let bet_id2 = CasinoHouse::place_bet(
            capability2, bet_coins2, @0x222, GAME_BET * 2
        );
        CasinoHouse::settle_bet(capability2, bet_id2, @0x222, GAME_BET / 2);
    ...
    fun test_precision_with_small_amounts() {
        ...
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(capability, bet_coins, @0x111, 200);
        CasinoHouse::settle_bet(capability, bet_id, @0x0, 0);
    ...
    fun test_state_consistency_after_failures() {
        ...
        let bet_coins = coin::withdraw<AptosCoin>(&casino_account, 1000);
        let capability = get_game_capability();
        let bet_id = CasinoHouse::place_bet(capability, bet_coins, @0x111, 1000);
        CasinoHouse::settle_bet(capability, bet_id, @0x111, 500);
